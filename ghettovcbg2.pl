...
my $VM_BACKUP_DATASTORE = "datastore1 (1)";
...
### ESXi 5.1 does not do 2gbsparse per default
my $DISK_BACKUP_FORMAT = "2gbsparse";
...
if(defined($configDir)) {
	...
	if( $VM_BACKUP_DATASTORE eq "" || $VM_BACKUP_ROTATION_COUNT eq "" || $DISK_BACKUP_FORMAT eq "" || $ADAPTER_FORMAT eq "" || $POWER_VM_DOWN_BEFORE_BACKUP eq "" || $LOG_LEVEL eq ""  ) {
		print "\nA required variable has not been defined, plesae go back and verify!\n";
		print '$VM_BACKUP_DATASTORE => ' . $VM_BACKUP_DATASTORE . "\n";
		print '$VM_BACKUP_DIRECTORY => ' . $VM_BACKUP_DIRECTORY . "\n";
		print '$VM_BACKUP_ROTATION_COUNT => ' . $VM_BACKUP_ROTATION_COUNT . "\n";
		print '$DISK_BACKUP_FORMAT => ' . $DISK_BACKUP_FORMAT . "\n";
		print '$ADAPTER_FORMAT => ' . $ADAPTER_FORMAT . "\n";
		print '$POWER_VM_DOWN_BEFORE_BACKUP => ' . $POWER_VM_DOWN_BEFORE_BACKUP . "\n";
...
if($vima_ver eq "1.0.0") {
	...
	@vima_targets = VIFPLib::enumerate_targets();
} elsif($vima_ver eq "4.1.0" || $vima_ver eq "5.1.0"){
	$vifs_cmd = "/usr/bin/vifs";
        $vmkfstools_cmd = "/usr/bin/vmkfstools";
		...
		if($success_backups{$vm_name} ne -1 && $success_backups{$vm_name} ne 1) {
			...
			if(defined($vm_view)) {
				...
				if($vm_view->snapshot) {
					...
					###fp
                                        #
                                        #$vmx_copy =  `$vifs_cmd --server "$host" --username "$host_username" --password "$host_password" --put "/etc/null" "[$VM_BACKUP_DATASTORE] $VM_BACKUP_DIRECTORY/$vm_name/$vm_name\-$VM_BACKUP_DIR_NAMING_CONVENTION/completed" 2>&1`;
                                        #&log("debug", "FP: completed ".$vmx_copy);
                                        #
                                        ###
					}
				}
...
###FP
                        &log("debug", "Snapshot state: " . $info->state->val);
###FP
			sleep 5;
